...
class ConsumatModel(Model):
    def __init__(self, config):
        ...
        self.tp_introduction_rate = config.get('tp_introduction_rate', 0.05)  # Default to 5% per step if not provided
        ...
        self.num_tp_products_introduced = 0  # Counter for the number of TP products introduced
        ...
        self.G = self.create_homophily_network(config['network_type'], config['network_params'])
    ...
    def create_homophily_network(self, network_type, params):
        ...
        if network_type == 'watts_strogatz':
            ...
            G = nx.powerlaw_cluster_graph(n=params['n'], m=params['m'], p=params['p'])
        else:
            raise ValueError(f"Unsupported network type: {network_type}")

        # Rewire edges based on homophily
        edges_to_rewire = []
        for u, v in G.edges():
            if not self.agents_similar(u, v):
                edges_to_rewire.append((u, v))

        for u, v in edges_to_rewire:
            G.remove_edge(u, v)
            new_v = self.find_similar_agent(u)
            G.add_edge(u, new_v)

        return G


    def agents_similar(self, agent_id1, agent_id2):
        attr1 = getattr(self.agents[agent_id1], self.homophily_attribute)
        attr2 = getattr(self.agents[agent_id2], self.homophily_attribute)
        return abs(attr1 - attr2) <= self.tolerance

    def find_similar_agent(self, agent_id):
        candidate_agents = [
            other_agent.unique_id for other_agent in self.agents
            if self.agents_similar(agent_id, other_agent.unique_id) and other_agent.unique_id != agent_id
        ]
    ...
    def step(self):
        ...
        # Introduce True Price products gradually over time
    ...
    def introduce_true_price_product(self):
        ...
        total_tp_products = int(len(self.products) * self.TP_percentage)
        num_new_tp_products = int(len(self.products) * self.tp_introduction_rate)

        # Ensure we do not introduce more TP products than the total desired percentage
        if self.num_tp_products_introduced + num_new_tp_products > total_tp_products:
            num_new_tp_products = total_tp_products - self.num_tp_products_introduced

        tp_product_indices = np.random.choice(
            [i for i in range(len(self.products)) if not self.products[i].is_true_price],
            num_new_tp_products,
            replace=False
        )
        ...
        self.num_tp_products_introduced += num_new_tp_products

        # Print the count of True Price products
        num_tp_products_available = self.count_true_price_products()
        print(f"Time step {self.schedule.steps}: {num_tp_products_available} True Price products available.")

    def count_true_price_products(self):
        return sum(1 for product in self.products if product.is_true_price) 
    ...
    def calculate_true_price_adoption_rate(self):
        ...
        true_price_users = sum(agent.is_using_true_price for agent in self.schedule.agents)
