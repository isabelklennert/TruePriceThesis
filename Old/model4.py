...
class Product:
    ...
    #def set_true_price(self):
        #self.is_true_price = True
        #self.tp_available = True
        #self.price_increase_percentage = self.calculate_price_increase()
       
    #def calculate_price_increase(self):
        # Inversely proportional to the green score
        #min_increase = 4  # Minimum price increase for the highest green score
        #max_increase = 10  # Maximum price increase for the lowest green score
        # Formula to calculate the price increase percentage based on the green score
        #return min_increase + (1 - self.green_score) * (max_increase - min_increase)
...
class ConsumatModel(Model):
    ...
    def create_sda_network(self, params):
        ...
        #Probability decreases as the distance increases, adjusted by alpha and b
        # Agents are more likely to connect if they are similar (close in social space)
        connection_probs = 1 / (1 + (b**-1 * distances)**alpha)
