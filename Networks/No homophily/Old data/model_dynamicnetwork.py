...
class ConsumatModel(Model):
    ...
    def create_agents(self):
        ...
        median_product_price = (low_price + high_price) / 2

        # Determine an average increase from True Price products to serve as a buffer factor
        buffer_factor = 1 + ((self.min_increase_percentage + self.max_increase_percentage) / 200)

        # Scale normalized incomes to fit the budget range with the buffer factor
        budgets = normalized_incomes * (high_price - low_price) * buffer_factor + low_price
