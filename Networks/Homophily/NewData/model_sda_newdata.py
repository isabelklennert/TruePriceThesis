...
class ConsumatModel(Model):
    ...
    def create_sda_network(self, params):
        ...
        n = params['n']
        alpha = params['alpha']  # Preferential attachment parameter
        b = params['b']  # Homophily parameter
        ...
        # Create an array of the homophily attribute for each agent
        social_space = np.array([getattr(agent, homophily_attribute) for agent in self.agents])

        # Calculate pairwise distances
        pairwise_distances = []
        for i in range(len(social_space)):
            for j in range(i+1, len(social_space)):
                pairwise_distances.append(np.abs(social_space[i] - social_space[j]))

        mean_dist = np.mean(pairwise_distances)

        # Calculate attachment probabilities
        prob_attach = np.zeros((n, n))
        k = 0
        for i in range(len(social_space)):
            for j in range(i+1, len(social_space)):
        ...
        # Build the graph based on the probability matrix
        np.random.seed(self.config['seed'])
        coin_flips = np.random.uniform(size=int(n * (n - 1) / 2))
        k = 0
        for i in range(len(prob_attach)):
            for j in range(i+1, len(prob_attach)):
                if prob_attach[i][j] > coin_flips[k]:
                    G.add_edge(i, j)
                k += 1
        ...
        largest_component_social_space = social_space[np.array(largest_component)]

        for i in range(1, len(connected_components)):
            comp = list(connected_components[i])
            node = np.random.choice(comp)
            closest_largest_component_node = (np.abs(largest_component_social_space - social_space[node])).argmin()
    ...
    def place_agents_on_grid(self):
        ...
        for node, agent in zip(self.G.nodes, self.agents):
