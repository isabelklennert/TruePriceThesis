...
class ConsumatModel(Model):
    def __init__(self, config):
        ...
        self.b = config['network_params']['b']
        self.alpha = config['network_params']['alpha']
        self.homophily_attribute = config['network_params']['homophily_attribute']
    ...
    def create_homophily_network(self):
        np.random.seed(self.config['seed'])
        random.seed(self.config['seed'])
        self.G = nx.Graph()
        self.G.add_nodes_from(range(self.num_agents))

        for i in range(self.num_agents):
            for j in range(i + 1, self.num_agents):
                agent1 = self.agents[i]
                agent2 = self.agents[j]
                prob = self.calculate_attachment_probability(agent1, agent2, self.b, self.alpha)
                if random.random() < prob:
                    self.G.add_edge(i, j)



    def calculate_attachment_probability(self, agent1, agent2, b, alpha):
        attr1 = getattr(agent1, self.homophily_attribute)
        attr2 = getattr(agent2, self.homophily_attribute)
        distance = abs(attr1 - attr2)
        probability = 1 / (1 + ((1/b) * distance) ** alpha)
        return probability
    ...
    def get_neighbors_for_agent(self, agent):
        # Use the network G to find neighbors for the given agent
        neighbors = list(self.G.neighbors(agent.unique_id))
        # Ensure we are referencing the correct agent objects from the schedule
        return [self.schedule.agents[neighbor] for neighbor in neighbors if neighbor < len(self.schedule.agents)]

    def print_network(self):
        print("Network structure:")
        for node in self.G.nodes:
            agent = self.schedule.agents[node]
            print(f"Node {node}: Agent ID {agent.unique_id}")
    ...
    def draw_network(self):
        ...
        nx.draw(self.G, pos, with_labels=True, node_color='lightblue', edge_color='gray', node_size=500, font_size=10)
    ...
    def create_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            #self.schedule.add(agent)
            #print(f'Created Agent {i} with budget {budget}')
            #print(f'Attributes: sustainability={row["impenv"]}, conformity={row["ipfrule"]}, job stability={row["lknemny"]}')
            #print(f'Weights: financial={weight_financial}, social={weight_social}, personal={weight_personal}')
    ...
    def get_neighbors_for_agent(self, agent):
        # Use the network G to find neighbors for the given agent
        neighbors = list(self.G.neighbors(agent.unique_id))
        return [self.schedule.agents[neighbor] for neighbor in neighbors]
